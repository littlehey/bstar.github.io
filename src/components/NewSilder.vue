  ...
  <div ref="wrap" :class="['vue-slider-component', flowDirection, disabledClass, { 'vue-slider-has-label': piecewiseLabel }]" v-show="show" :style="wrapStyles" @click="wrapClick">
    <div ref="elem" aria-hidden="true" class="vue-slider" :style="[elemStyles, bgStyle]">
      <template v-if="isRange">
        <div ref="dot0" :class="[tooltipStatus, 'vue-slider-dot']" :style="[dotStyles, sliderStyles[0]]" @mousedown="moveStart(0)" @mouseover="overStart(0)" @mouseout="overEnd(0)" @touchstart="moveStart(0)">
        ...
        <div ref="dot1" :class="[tooltipStatus, 'vue-slider-dot']" :style="[dotStyles, sliderStyles[1]]" @mousedown="moveStart(1)" @mouseover="overStart(1)" @mouseout="overEnd(1)" @touchstart="moveStart(1)">
      ...
      <template v-else>
        <div ref="dot" :class="[tooltipStatus, 'vue-slider-dot']" :style="[dotStyles, sliderStyles]" @mousedown="moveStart" @mouseover="overStart" @mouseout="overEnd" @touchstart="moveStart">
      ...
      <ul class="vue-slider-piecewise">
        <li v-for="(piecewiseObj, index) in piecewiseDotWrap" class="vue-slider-piecewise-item" :style="[piecewiseDotStyle, piecewiseObj.style]" :key="index">
          <slot name="piecewise" :label="piecewiseObj.label" :index="index" :first="index === 0" :last="index === piecewiseDotWrap.length - 1">
            <span v-if="piecewise" class="vue-slider-piecewise-dot" :style="[ piecewiseStyle, piecewiseObj.inRange ? piecewiseActiveStyle : null ]"></span>
          </slot>

          <slot name="label" :label="piecewiseObj.label" :index="index" :first="index === 0" :last="index === piecewiseDotWrap.length - 1">
            <span v-if="piecewiseLabel" class="vue-slider-piecewise-label" :style="[ labelStyle, piecewiseObj.inRange ? labelActiveStyle : null ]">
              {{ piecewiseObj.label }}
            </span>
          </slot>
        </li>
...
export default {
    ...
    wrapStyles() {
      return this.direction === 'vertical' ? {
        ...
        // padding: `${this.dotHeightVal / 2}px ${this.dotWidthVal / 2}px`
          ...
          width: typeof this.width === 'number' ? `${this.width}px` : this.width // eslint-disable-line
          // padding: `${this.dotHeightVal / 2}px ${this.dotWidthVal / 2}px` // eslint-disable-line
        } // eslint-disable-line
    },
    sliderStyles() {
    ...
    elemStyles() {
      return this.direction === 'vertical' ? {
        ...
        height: '100%'
          ...
          height: `${this.height}px` // eslint-disable-line
        } // eslint-disable-line
    },
    dotStyles() {
      return this.direction === 'vertical' ? {
        ...
        left: `${(-(this.dotWidthVal - this.width) / 2)}px`
          ...
          width: `${this.dotWidthVal}px`, // eslint-disable-line
          height: `${this.dotHeightVal}px`, // eslint-disable-line
          top: `${(-(this.dotHeightVal - this.height) / 2)}px` // eslint-disable-line
        } // eslint-disable-line
    },
    piecewiseDotStyle() {
      return this.direction === 'vertical' ? {
        ...
        height: `${this.width}px`
          ...
          width: `${this.height}px`, // eslint-disable-line
          height: `${this.height}px` // eslint-disable-line
        } // eslint-disable-line
    },
    piecewiseDotWrap() {
      ...
      for (let i = 0; i <= this.total; i++) {
        const style = this.direction === 'vertical' ? {
          ...
          left: 0
            ...
            left: `${this.gap * i - this.height / 2}px`, // eslint-disable-line
  ...
  methods: {
    ...
    overStart(index) {
      ...
      this.$emit('over-start', this)
    },
    overEnd(index) {
      if (this.isDisabled) return false
      this.$emit('over-end', this)
    },
    moveStart(index) {
      if (this.isDisabled) return false
    ...
    setTransform(val) {
      const value = (this.direction === 'vertical' ? ((this.dotHeightVal / 2) - val) : (val - (this.dotWidthVal / 2))) * (this.reverse ? -1 : 1)
